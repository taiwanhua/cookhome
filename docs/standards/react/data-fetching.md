...
登入者資料一律經 `features/auth/use-me` 的 `useMe()`(`me` query 的唯一包裝,`staleTime: Infinity`、**`retryOnMount: false`**)。`retryOnMount` 必須關:`me` 失敗(如 token 失效)後 `RequireAuth` 會導向登入頁並重新掛載,預設 `retryOnMount: true` 會在掛載時再打一次 → 再失敗 → 再導向,形成無限迴圈(#65 踩過)。其他「失敗就導向」的查詢比照。
