...
- **回饋循環**:review 中被採納的新決定,回寫到對應檔案成為新規則(沿用編號流水號);規則被推翻時直接改寫該條(不註記日期、票號,歷史查 git;STRUCT-11),不留殭屍規則。
...
| 測試                                                | `testing/testing.md`(api 整合測試 TEST-07、admin 元件測試與 mock 開發模式 TEST-08、ui 元件測試 TEST-09、劇本 E2E TEST-05 / TEST-11) |
