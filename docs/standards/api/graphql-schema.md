...
| `USER_NOT_ELIGIBLE`        | 授予資格未過:該使用者的所屬組織皆不在角色擁有組織的子樹內(ADR-0003)。**兩個入口同碼**(2026-09-21 / #261):角色頁的 `grantRoleUsers` 與使用者頁的 `assignUserRoles`(後者原為 `VALIDATION_FAILED`);`extensions.roleId` / `ownerOrgName` 指出是哪個角色與它的擁有組織 | 顯示「此角色只能授予 <ownerOrgName> 及其下層的使用者」,並把該列設為 disabled     |

錯誤的 `message` 給開發者看(英文);給使用者的繁體中文文案由前端依 code 對應,不從 api 傳。

**`FORBIDDEN` 的 `extensions.reason`**(2026-09-21 / #261):`FORBIDDEN` 是通用碼,光看碼分不出「為什麼不行」,
所以**受規則保護**的那幾種情形另附 `reason`,前端據此顯示不同的一句話。這是 `reason` 不是新的 `code`,
本表不因此增列:
