...
- **`form_submissions.touched[]`**(#482):使用者碰過的欄位 key(預設為空陣列),存草稿時由前端一併送來;沒碰過的欄位在草稿填寫時會隨依賴重算預設值,碰過就停。只對草稿有意義,送出後不再變動。`values` 的 `datetime` 欄存 ISO 8601 UTC(`YYYY-MM-DDTHH:mm:ssZ`)。欄位定義(`form_versions.fields[]`)另可帶 `default`(預設值)與上傳欄的 `widget.accept` / `widget.maxSizeMb`(形狀正本 `@repo/domain/form` 的 `types.ts`)。
