...
- **`form_submissions` 的 6b 欄位**(`form-submission.schema.ts`):`status` 七值(`draft` / `reviewing` / `returned` / `withdrawn` / `completed` / `rejected` / `voided`,正本 `@repo/domain/workflow` 的 `SUBMISSION_STATUSES`);`currentInstanceId` 為 null = 沒走過流程(`completed` 後可修改),有值 = 走過(`completed` 後鎖定、只能作廢);`blocked` = 實例被阻擋;`voidedAt` / `voidedBy` / `voidReason` / `replacedById` 是作廢資訊;`copiedFrom` = 這筆草稿是「複製為新單」從哪一筆複製來的(一般新增為 null)。
- **`form_submissions.touched[]`**:使用者碰過的欄位 key(預設為空陣列),存草稿時由前端一併送來;沒碰過的欄位在草稿填寫時會隨依賴重算預設值,碰過就停。只對草稿有意義,送出後不再變動。`values` 的 `datetime` 欄存 ISO 8601 UTC(`YYYY-MM-DDTHH:mm:ssZ`)。欄位定義(`form_versions.fields[]`)另可帶 `default`(預設值)與上傳欄的 `rules.accept` / `rules.maxSizeMb`(形狀正本 `@repo/domain/form` 的 `types.ts`)。
- **`form_versions` 的草稿是硬刪**(`deleteFormVersionDraft`;ADR-0007 軟刪的例外):`(formKey, status)` 的部分唯一索引含軟刪除的文件,軟刪的草稿會讓這張表單永遠開不了新草稿;草稿從未發布、沒有提交綁它,硬刪沒有懸空引用,刪掉的整份定義留在稽核 `form-version.delete-draft` 的 `before`。
...
- **`workflow_versions`** 與 `form_versions` 同一套部分唯一索引(版號只在是數字時唯一、`draft` / `publishing` 各至多一筆);`steps[]` / `edges[]` 存自由 JSON,形狀正本是 `@repo/domain/workflow` 的 `StepDef` / `WorkflowEdge`(沒有 `edges` = 直線,依 `steps[]` 順序)。`checkFormKey`(選填,表單 key)是設計器的「檢查用表單」:存草稿一併存、發布快照保留、開草稿 / fork 帶過去,只影響設計時的檢查。刪除草稿是**硬刪**(ADR-0007 軟刪除的例外:草稿從未發布、沒有引用,軟刪除會佔住「至多一份草稿」的部分唯一索引);刪前內容留在稽核 `workflow-version.delete-draft` 的 `before`。
- **`workflow_instances` 是唯一權威**(`workflow-instance.schema.ts`):**建立時就為版本的每個節點各建一筆 `pending` 的 `StepState`**(`@repo/domain/workflow` 的 `initialStepStates`),之後只改欄位、不增刪元素 —— 推進的條件更新是對 `steps` 陣列元素下條件(`$elemMatch` / `arrayFilters`),元素不存在就永遠不成立。`steps[]` 每關一筆 `StepState`(`stepKey` / `status` / `blocked` / `plan[]` 派任計畫 / `decisions[]` 已接受的決定,陣列順序 = 接受順序)、`activeStepKeys[]`、`editVersion`(每次狀態改變 +1,CAS 用)、`outcome`(全案終局採用的那筆決定,只能從 null 寫一次)、`history[]`(`notified` 事件帶 `result` 當通知標記)、`linkSource`(送出時依據的草稿與流程版本)、`summary`(該修訂的摘要槽快照,任務列表 / 通知讀它)。唯一 `(submissionId, revision)`、索引 `(tenantId, status)`;模組資料表的四個基礎欄位抄自提交,資料範圍規則不套(沒有宣告目標)。任務只是投影,狀態由 `@repo/domain/workflow` 的 `advance` 判斷表依實例同步。
