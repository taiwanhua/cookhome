...
表單綁了審核流程時,送出後進「審核中」、核准才「已完成」並鎖定(狀態共七個),見 `workflow-engine.md`。
...
- 值依型別存(選項 `{ value, label }`、引用 `{ id, label }`、上傳 `{ path, name, size, contentType }`、數字十進位字串、日期時間 ISO 8601 UTC 以租戶時區輸入與顯示);表達式看到的是語意值(選項的 value、引用的 id)。
