...
| 表單版本(頁籤)         | 草稿與各版本、發布(changelog 必填)、發布中斷重試、退役目前版本、與上一版差異、以任一版本為基底開新草稿                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- 表單編輯跳窗的頁籤模板下方即時顯示以範例摘要套用的結果(`lib/form-engine/tab-label.ts` 的 `applyTabLabelTemplate`,留空以預設模板示範)。
- 設計器的表達式一律用**型別導向的結構化選擇器**(欄位 / 系統值 / 常數 / 運算,可巢狀),不做文字輸入:每個位置帶期望型別往下傳,只列型別對得上的東西(型別表正本 `@repo/domain/form` 的 `expression-types.ts`,過濾在 `lib/form-engine/expression-options.ts`)。公式的根 = 欄位型別;條件(顯示 / 鎖定條件、自訂驗證、流程跳過條件)的根 = 是 / 否,常數與系統值不能單獨當條件的根;條件不列受保護欄位。`dateDiff` 節點有單位下拉(天 / 小時 / 分鐘,預設天),產生第三參數。
