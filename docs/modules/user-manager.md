...
- **擁有者保護只需要一個 id**:前端不重做 `owner-protection.service.ts` 的判斷,只取
  ...
  它的 `ownerUserId` 就是受保護的那一位,該列的「停用」disabled 並提示。
  **「所屬組織」照常可開**(2026-09-22 / #362 修正):api 的 `assertOwnedOrgsKept` 只擋
  「把擁有者移出他擁有的租戶頂層」,加入其他組織一直是允許的 —— 整個按鈕 disabled 比 api 嚴,
  擁有者因此連把自己加進分店都做不到。鎖改在彈窗裡:樹根(= 他擁有的租戶頂層)那一個節點
  以 `Tree` 的 `disabledCheckIds` 鎖住(維持勾選、取消不掉),其餘組織照常可勾可取消。
  **「哪個組織是他擁有的」不必向 api 多要欄位** —— `protectedOwnerUserId` 本來就是從樹根的
  `ownerUserId` 來的,所以樹根就是那一個組織(`protectedOwnerOrgId`)。
