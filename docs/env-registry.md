...
|---|---|---|---|---|---|
| `MONGODB_URI` | DB 連線(含帳密) | api runtime、db-migrator、CI 測試 | `mongodb+srv://…/<db>` | `mongodb-uri*` / Cloud Run compute SA | 已設、已接線 |
...
| `ROOT_ADMIN_PASSWORD` | seed 建立首個超管的初始密碼(ADR-0002;只在帳號不存在時用) | db-migrator seed(CI 步驟) | 字串 | `root-admin-password*` / `github-deployer` SA | 已設;接線待 deploy.yml 加 seed 步驟(#28 合入後) |
...
| 變數 | 用途 | 用於 |
|---|---|---|
| `ROOT_ADMIN_ACCOUNT` / `ROOT_ADMIN_EMAIL` | seed 首個超管的帳號與信箱(ADR-0002);帳號定為 `root` | db-migrator seed |
