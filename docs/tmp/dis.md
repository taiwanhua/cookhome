...
18. **模組文件系統(2026-09-07 定案)** — 每模組**兩份**檔案:`docs/modules/<module-key>.md`(內部技術/業務文件:流程、規則、建模結論)+ `docs/modules/<module-key>.help.md`(**給使用者看的說明**,語氣白話、守詞彙表);後者 seed 時灌進 module 種子,AppBar 標題旁「?」彈窗渲染。兩份受眾不同、內容不同,但同目錄同 PR 維護。
