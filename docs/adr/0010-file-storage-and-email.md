...
- `StorageService` 介面帶 `visibility: 'private' | 'public'` 參數 — 新模組要存檔只選參數,不再碰基建

### 上傳(私有/公開共用同一條路)

前端向 API 要 **V4 上傳簽名網址**(API 先驗:操作者權限、檔型 PNG/JPG、大小 ≤2MB;網址效期 10 分鐘)→ 瀏覽器**直傳 GCS** → API 寫回物件路徑。檔案不經過 API server。

### 讀取(私有與公開唯一的分岔)

- **私有**:前端經 API 取檔 — API 驗權(接資料隔離與 data-scope:BaseRepository 查得到該筆資料的人才可取檔)後,簽發**短效讀取簽名網址**(商標類 TTL 1 小時、前端快取;敏感檔約 15 分鐘);極敏感檔案可收緊為 API 代理串流。**DB 存物件路徑(bucket+key),不存 URL**(簽名網址會過期,看時現簽)。
- **公開**:穩定公開 URL,供 CDN 快取、SEO、og:image;DB 可直接存 URL。

### 新模組怎麼選(建模組流程必問,dis.md #16)

**預設私有**。檔案需要 CDN 快取、SEO、對未登入者展示或社群分享 → 才選公開。私有檔案同時確認讀取權限對應哪個 data-scope。底座唯一的圖(組織商標,路徑 `orgs/<orgId>/logo.<ext>`,覆蓋即換圖)只在登入後的後台顯示 → 私有。
