...
import { routeTabsStorageKey } from "./route-tabs-store";

const STORAGE_KEY = routeTabsStorageKey(testUser.id);

async function findTabList() {
  return screen.findByRole("tablist", { name: "路由頁籤" });
}
...
describe("RouteTabs:生成與去重(dis #15:開過的路由生成 tab、以路由去重)", () => {
  ...
  it("點 tab 切換:內容區與網址同步、AppBar 標題同步", async () => {
    ...
    await user.click(sideNavLink("組織管理"));
    expect(
      await screen.findByRole("heading", { name: "組織管理" }),
    ).toBeInTheDocument();
...
describe("RouteTabs:關閉", () => {
  it("關閉非當前 tab:當前頁與網址不變", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "組織管理" }),
    ).toBeInTheDocument();
  });
