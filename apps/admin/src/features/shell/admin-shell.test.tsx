...
async function findSideNav() {
  return screen.findByRole("navigation", { name: "主選單" });
}

describe("SideNav(模組陣列以 parentId 組樹;ADR-0011「前端判斷 / 側欄」)", () => {
  it("超級管理員:系統管理群組六項 + 示範群組(含次群組),依 order 排序,hidden 模組不出現", async () => {
    server.use(
  ...
  it("只綁示範模組2 的角色:只看到示範群組,沒有系統管理", async () => {
    server.use(
...
describe("路由防守與佔位頁(ADR-0011「路由防守」:可進 = 有那個模組路由)", () => {
  it("手打 /system/org-manager 而模組陣列無此模組 → 無權限頁", async () => {
    server.use(
      ...authWorld({ hasRefreshCookie: true, modules: sampleTwoModules })
        .handlers,
    );
